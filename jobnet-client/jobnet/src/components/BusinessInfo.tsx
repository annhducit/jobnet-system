...
import {
  ...
  getBusinessBackgroundImage,
  updateBusinessFollowers,
...
import { updateJobSeekerBusinessFollowed } from 'User/JobSeeker/services'
...
export default function BusinessInfo({
  ...
  const handleFollowClick = () => {
    void (async () => {
      if (!jobSeekerId) {
        toast.error(t('toast.errors.requireLogin'))
        return
      }
      const jobSeekerRes =
        await updateJobSeekerBusinessFollowed(jobSeekerId, {
          status: 'FOLLOW',
          businessId: businessId,
        })
      if (!jobSeekerRes) {
        return toast.error(t('toast.errors.common'))
      }
      const business = await updateBusinessFollowers(businessId, {
        status: 'FOLLOW',
        userId: jobSeekerId,
      })
      if (business) {
    ...
    <div className={clns(className, 'space-y-4')}>
      <div>
        <div className="relative h-56">
          <img
            src={getBusinessBackgroundImage(business.id)}
        ...
        <div className="relative flex flex-col items-center gap-8 p-8 border rounded-b lg:flex-row bg-second-lower">
          {mode === 'update' ? (
            <div className="relative">
              <img
                src={getBusinessProfileImage(business.id)}
