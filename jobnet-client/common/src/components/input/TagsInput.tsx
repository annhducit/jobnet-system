...
export default function TagsInput<T, K>({
  id,
  label,
  name,
  placeholder,
  ...
  const tagElms = tagsInput.tags.map((tag, i) => (
    <div
      key={tag.id}
      className="flex items-center gap-1 pl-2 text-sm leading-none text-white rounded bg-main"
    >
      {tag.name}
      <div
        className="px-2 py-1 cursor-pointer hover:text-black"
  ...
  const hintElms = tagsInput.hints.map((hint, i) => {
    ...
    return (
      <div
        key={hint.id}
        className={clns('p-2 hover:bg-main hover:text-white', {
          'bg-main-lower text-white': isSelected,
        })}
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        data-id={id}
        className={clns(
          'relative flex flex-wrap min-h-[41.6px] items-center gap-2 p-2.5 border rounded-lg border-main-lower',
          {
            'ring-2 ring-main border-transparent': tagsInput.isFocus,
          }
        )}
      >
        {tagElms}
        ...
        />
        <FaTags
          className="absolute w-5 h-5 cursor-pointer top-3 right-2 text-second hover:text-main"
          onClick={handleTagAdd}
        />
        {!!tagsInput.hints.length && tagsInput.isFocus && (
          <div className="absolute left-0 z-50 w-full py-2 overflow-y-scroll text-sm bg-white border-2 rounded shadow-md max-h-48 q top-full border-second-lower">
            {hintElms}
          </div>
        )}
      </div>
