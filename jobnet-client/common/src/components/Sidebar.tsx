...
const theme = {
  sidebar: {
    base: ' h-full p-4 bg-primaryLower text-sm transition-all',
    icon: {
      base: 'w-5 h-5 text-second',
      collapse: {
        on: '',
        off: 'mr-4',
      },
  ...
  logo: {
    base: 'flex items-center mb-5 text-2xl font-bold text-second',
    img: {
      base: 'text-second',
      collapse: {
        on: 'h-9',
  ...
  item: {
    base: 'flex items-center justify-between p-2 rounded font-semibold transition-all',
    active: {
      on: 'bg-main text-white',
      off: 'text-slate-900 hover:bg-mainLower',
    },
    itemCollapse: 'pl-5',
  },
}
  ...
  return (
    ...
    >
      <div className={baseUtils}>
        <div
          className="absolute p-1 rounded top-2 -right-7 bg-second-lower"
          onClick={handleChangeCollapse}
        >
          <CollapseIcon className={`${iconUtils} z-[99999999]`} />
        </div>
        {children}
      </div>
    </SidebarContext.Provider>
...
Sidebar.Logo = function SidebarLogo({
  ...
  const baseUtils = classnames(className, theme.logo.base)
  const imgUtils = classnames(
    theme.logo.img.base,
...
Sidebar.ItemGroup = function SidebarItemGroup({
  ...
  const baseUtils = classnames(className, theme.itemGroup.base)
...
Sidebar.Item = function SidebarItem({
  ...
  const activeUtils = classnames(baseUtils, theme.item.active.on)
  const noActiveUtils = classnames(baseUtils, theme.item.active.off)
...
Sidebar.Collapse = function SidebarCollapse({
  ...
  const baseUtils = classnames(className, theme.item.base)
  const iconUtils = classnames(
    theme.sidebar.icon.base,
    isCollapse
      ? theme.sidebar.icon.collapse.on
