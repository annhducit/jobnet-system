  ...
  return (
    <div
      className={`flex items-start justify-center ${
        layoutSize === 'lg' ? 'py-1' : 'h-screen md:pt-14 '
      } md:px-2 lg:px-0 md:bg-mainLower`}
    >
      <div
        className={`${
          layoutSize === 'xs'
            ...
            ? 'max-w-[1050px]'
            : layoutSize === 'lg'
            ? 'max-w-screen h-screen overflow-y-scroll pr-2'
            : ''
      ...
      >
        {!verify && (
          <div className="relative hidden py-4 bg-gray-100 rounded-md md:block">
            ...
            <div
              className={`${
                layoutSize === 'lg' ? 'space-y-56' : 'gap-y-10'
              } px-6 pt-28`}
            >
        ...
        <div className={`${padding} ${layoutSize === 'lg' && 'pr-4'}`}>
          <p className="text-2xl font-bold text-main">{welcome}</p>
          <p className="pt-2 font-semibold text-gray-700">{introduction}</p>
          <p className={`${padding} text-lg font-semibold`}>{intendedFor}</p>
          {children}
        </div>
      </div>
    </div>
