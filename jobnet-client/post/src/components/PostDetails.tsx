...
export function PostDetailsInfo({
  ...
  const locationElms = post.locations.map((location, index) => (
    <p key={index}>{location.provinceName}</p>
  ))

  return (
    <div className="space-y-4">
      <PostDetailsSection>
        <div className="col-span-1 p-6 bg-white border rounded">
          <div className="flex flex-col gap-y-4">
            <div className="flex gap-x-4">
              <img
                src={`http://localhost:3011${businessProfile}`}
                className="object-cover w-24 h-24 border rounded border-mainBorder"
              />
        ...
        <div className="p-6 col-span-2 h-[auto] border rounded bg-white">
          <div className="flex flex-col gap-y-2">
      ...
      <PostDetailsSection>
        <div
          className={clns('col-span-1 p-6 rounded', {
            'bg-white border h-auto': type === 'Update' || type === 'Preview',
